...
const { canPublishCourseToUser } = require('../domain/learningRules');
...
const getCourseCounts = async (courses) => {
  ...
  const lessonCountByCourse = new Map(lessonCounts.map(item => [String(item._id), item.total]));
  const enrollmentCountByCourse = new Map(enrollmentCounts.map(item => [String(item._id), item.total]));

  return courses.map(course => {
    const lessonCount = lessonCountByCourse.get(String(course._id)) || 0;
    const enrollmentCount = enrollmentCountByCourse.get(String(course._id)) || 0;
    return {
      ...course,
      lessonCount,
...
const listCourses = catchAsync(async (req, res) => {
  ...
  if (req.currentUser?.role === 'teacher') {
    filter.$or = [{ status: { $ne: 'draft' } }, { teacher: req.currentUser._id }];
