...
const LmsView = ({
  ...
  const handleTextSubmit = async (assignmentId) => {
    ...
    try {
      ...
      if (pendingFile) {
        console.log('[LmsView] Uploading video before submit:', pendingFile.name)
        fileUrl = await onUploadSubmissionVideo?.(pendingFile) || ''
        console.log('[LmsView] Upload result fileUrl:', fileUrl)
      }

      // Now submit assignment with content + fileUrl
      console.log('[LmsView] Submitting assignment:', assignmentId, 'fileUrl:', fileUrl)
