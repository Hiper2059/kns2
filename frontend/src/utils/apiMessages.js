...
const friendlyFromText = (message, status) => {
  ...
  if (normalized.includes('noi dung') && normalized.includes('rong')) {
    return 'Nội dung không được để trống.'
  }

  if (normalized.includes('thieu title') || normalized.includes('thieu tieu de')) {
...
export const getApiSuccessMessage = (responseOrMessage, fallback = 'Thao tác thành công.') => {
  const raw =
    typeof responseOrMessage === 'string'
      ? responseOrMessage
      : responseOrMessage?.data?.message || responseOrMessage?.message
  const repaired = repairMojibake(raw).trim()
  const normalized = normalizeMessage(repaired)

  if (successMessages.has(normalized)) {
...
export const getApiErrorMessage = (errorOrMessage, fallback = 'Có lỗi xảy ra. Bạn thử lại sau nhé.') => {
  const status = errorOrMessage?.response?.status || 0
  const raw =
  ...
  if (!errorOrMessage?.response && typeof errorOrMessage !== 'string' && raw) {
  ...
  const friendly = friendlyFromText(raw, status)
  return friendly || fallback
}

export const sanitizeApiError = error => {
  if (error?.response?.data && typeof error.response.data === 'object') {
    const fallback = error.response.status >= 500 ? 'Hệ thống đang bận. Bạn thử lại sau nhé.' : undefined
    error.response.data.message = getApiErrorMessage(error, fallback)
...
export const sanitizeApiResponse = response => {
  if (response?.data && typeof response.data === 'object' && typeof response.data.message === 'string') {
    response.data.message = getApiSuccessMessage(response)
  }
  return response
}
